...
| 近场/接触抓取 | D405 已在机器人 USB 3.x 链路持续采集 20 帧 `848x480@15`、达到 16.25 Hz；双压力贴片评估和有界闭合/保持/单次释放闭环已实现，但压力 USB 映射及当前目标/Jacobian 标定仍缺失 |
...
- **D405 流已通过真机验收，接触执行尚未通过。** 压力采集和有界抓取循环已实现；仍需
  确认压力 Frame 字段、贴片映射、阈值和 D405 Target/Jacobian，之后 `--go` 才能放行。
