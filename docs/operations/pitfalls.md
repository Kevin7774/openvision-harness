...
> | `scripts/xr1_verify.py` | **不存在了**。等价检查:`bin/xr1 preflight` ＋ `bin/tf-frames`；外置录像器仅手动可选 |
...
| **修法** | 当前右臂规划器开工前应在人工许可后运行 `bin/xr1 ready --side right`，进入实测规划起始位。不要用 `home`：零位夹垫 z=0.3527 m，低于 0.785 m 的规划 Floor Gate，会让所有候选失败关闭。左臂没有对应实测位，不要镜像猜测 |
| **这解释了一条老账** | 机制仍然是两次探测之间手臂停的位形不同，同一个点可能一次有路、一次无路。旧流程曾靠归零得到可比结果；当前流程改为固定的 `ready` 位后再观测和规划 |
