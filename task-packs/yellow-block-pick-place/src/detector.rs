//! The measured yellow-block detector.
//!
//! This is the canonical home of the colour thresholds Step 1 found living in
//! `xr1-vision/src/perception/yellow.rs`. The maths, the thresholds, and the
//! regression tests are moved here unchanged — `perception/yellow.rs` now
//! re-exports these functions, so the three call sites (`observe_object`, the
//! servo target mask, the D405 near-field target) and their behaviour are
//! identical. What changed is *ownership*: a different object's detector is a
//! different task pack, not an edit to the core.

use std::collections::VecDeque;

pub fn component_mask(rgb: &image::RgbImage) -> Vec<bool> {
...
pub fn components(rgb: &image::RgbImage) -> Vec<Vec<usize>> {
