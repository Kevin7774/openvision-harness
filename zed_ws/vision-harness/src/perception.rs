...
fn is_yellow(r: u8, g: u8, b: u8) -> bool {
    ...
    let gf8 = g as f64;
    let bf8 = b as f64;
    let dark_chromatic_yellow = rf8.max(gf8) >= 35.0
        && rf8.min(gf8) - bf8 >= 12.0
        && (rf8 - gf8).abs() <= 0.35 * rf8.max(gf8)
        && rf8 >= gf8
        && gf8 >= 0.75 * rf8;
    ...
    let delta = max - min;
    if max < 0.28 || delta < 0.20 {
        return dark_chromatic_yellow;
    }
    let hue = if max == rf {
    ...
    dark_chromatic_yellow
        || ((38.0..=72.0).contains(&hue) && delta / max >= 0.35)
}
